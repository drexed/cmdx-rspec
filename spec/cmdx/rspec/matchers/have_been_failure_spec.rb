...
RSpec.describe "have_been_failure matcher" do
  ...
  describe "failure attributes validation" do
    context "with default failure result" do
      ...
      it "validates state is INTERRUPTED" do
        expect(result).to have_been_failure(state: CMDx::Result::INTERRUPTED)
      end

      it "validates status is FAILED" do
        expect(result).to have_been_failure(status: CMDx::Result::FAILED)
      end

      it "validates cause is a FailFault" do
        expect(result).to have_been_failure(cause: be_a(CMDx::FailFault))
      end
    end
  end
  ...
  describe "edge cases" do
    context "with non-CMDx::Result input" do
      it "raises ArgumentError for string" do
        expect { expect("not a result").to have_been_failure }.to raise_error(ArgumentError, "must be a CMDx::Result")
      end

      it "raises ArgumentError for nil" do
        expect { expect(nil).to have_been_failure }.to raise_error(ArgumentError, "must be a CMDx::Result")
      end

      it "raises ArgumentError for arbitrary object" do
        expect { expect(Object.new).to have_been_failure }.to raise_error(ArgumentError, "must be a CMDx::Result")
      end
    end
  end
  ...
  describe "matcher description" do
    it "provides clear description" do
      expect(have_been_failure.description).to eq("have been a failure")
    end
  end
end
