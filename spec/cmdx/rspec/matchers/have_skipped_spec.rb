...
RSpec.describe "have_skipped matcher" do
  ...
  describe "edge cases" do
    context "with non-CMDx::Result input" do
      it "raises ArgumentError for string" do
        expect { expect("not a result").to have_skipped }.to raise_error(ArgumentError, "must be a CMDx::Result")
      end

      it "raises ArgumentError for nil" do
        expect { expect(nil).to have_skipped }.to raise_error(ArgumentError, "must be a CMDx::Result")
      end

      it "raises ArgumentError for arbitrary object" do
        expect { expect(Object.new).to have_skipped }.to raise_error(ArgumentError, "must be a CMDx::Result")
      end
    end
  end
  ...
  describe "matcher description" do
    it "provides clear description" do
      expect(have_skipped.description).to eq("have been skipped")
    end
  end
end
